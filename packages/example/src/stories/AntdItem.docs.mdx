  ...
  data={[
    {
      desc: (
        <>
          当表单字段接收类型受异步 <Tag>schema</Tag> 约束时需要
        </>
      ),
      keyname: 'async',
    ...
    },
    {
      desc: (
        <>
          表单字段接收的数据类型，不提供时接收的数据类型为 <Tag>unknown</Tag>
        </>
      ),
      keyname: 'schema',
    ...
    },
    {
      desc: (
        <>
          和 <Tag>useEventChat</Tag> 中的选项 <Tag>callback</Tag>
          一样。除此之外，回调函数中会追加字段的 <Tag>emit</Tag> 作为第二个参数，便于继续相应更新。
        </>
      ),
      keyname: 'callback',
    ...
    },
    {
      desc: (
        <>
          和 <Tag>useEventChat</Tag> 中的选项 <Tag>debug</Tag>{' '}
          一样，调试时用于收集所有为匹配字段规则的消息
        </>
      ),
      keyname: 'debug',
      type: ['Function', 'undefined'],
    },
    {
      desc: (
        <>
          通过 <Tag>emit</Tag> 触发的字段更新，会将 <Tag>callback</Tag> 回调参数中的{' '}
          <Tag>detail</Tag>，以及字段本身的 <Tag>emit</Tag> 通过 <Tag>onChange</Tag>{' '}
          透传回来，便于继续相应更新。
        </>
      ),
      keyname: 'onChange',
...
原来的 `Form.Item` 的子集如果是一个函数时，会将 `FormInstance` 作为回调函数。由于拓展了实例 `FormInstance`，回调函数中 `form` 实例类型为 `FormInsType` [[查看](tag2)]。
...
由 `Form.useForm` 或 `Form.useFormInstance` 返回的 `form` 实例，拓展实例的属性如上所述。为了兼容 `Antd v5` 和 `Antd v6`，将实例中的方法 `focusField` 作为可选属性，由业务环境依赖决定。
...
使用 `Schema` 让字段接受更复杂的数据类型，这里有两种方式：

- 根据字段值渲染对应的组件，但不会修改值，可以通过 `Antd` 原有的属性 `getValueFromEvent` 以及 `getValueProps` 实现；
- 根据收到的字段值，转换数据类型。可以通过 `Zod` 的 `transform` 来实现。如上面演示，`emit` 提供数值，字段接收后会将其转换成一个对象。

以上两个字段都只接收数值类型，`rateInput` 不会过滤数据，而 `transform` 会做转换。通过这样的方式，相同的字段，相同的组件，可以根据业务灵活配置需要的数据。而发起更新的 `emit` 仍旧保持数值类型不变，不会随着业务或者处理逻辑不同而改变。

> 遗留问题：`Form.Item` 的属性 `shouldUpdate` 请通过 `Antd` 原本的 `Form.Item`，目前扩展的表单项使用之后不会刷新表单项。
