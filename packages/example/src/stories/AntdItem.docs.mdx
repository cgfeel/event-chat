...
<Table className={{ wrap: 'sb-unstyled' }} maxHeight={200} stickyHeader>
  <Thead>
    <Tr>
      <Th className="bg-gray-100" sticky>
        更新场景
      </Th>
      <Th>
        <Tag type="info">Form.Item</Tag> 扩展属性
      </Th>
      <Th>表单组件属性</Th>
    </Tr>
  </Thead>
  <TBody>
    ...
    <Tr>
      <Td sticky>①自然输入</Td>
      <Td>
        不会触发扩展的 <Tag>onChange</Tag> 回调，但会将 <Tag>schema</Tag> 转换成一条 <Tag>Rule</Tag>
...
> 由于触发的场景不一样，校验逻辑也不一样，为了避免混淆没有合并 `onChange` 回调方法。
