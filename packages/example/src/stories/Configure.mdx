...
- 本地跨域、跨线程通信，可三方对接，可减少线程通信心智负担聚焦业务本身

## 文档阅读补充

- `@event-chat/core` 和 `@event-chat/antd` 放在一起说明，`@event-chat/rpc` 单独一篇演示说明
- 在文档所有属性介绍的表格中，若属性名后面没有符号： <font color="red">\*</font>，说明是可选参数允许为 `undefined`，而类型说明时不再列举 `undefined`
